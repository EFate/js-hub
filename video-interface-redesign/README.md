...
| 模板广告位 | 服务端渲染的 `div[data-slots]` 占位容器（+CSS 兜底隐藏） | 移除 |
| 静态图片广告 | 跨站绝对外链 + 内容仅为图片（视频站内容卡均为站内相对链接） | 隐藏 |
...
- **修复 PC 模板不激活**：PC 卡片结构为 `li > p.img > img`（`a` 为空覆盖层与 img 平级），原 `a img` 选择器数出 0 张卡 → 5 分卡门槛。卡片指纹改为全页 img 逐级上探找「含图+日期」容器，PC 实测 5 → 8 分自动激活；
- **新增通道 D**：移除模板广告系统的服务端占位容器 `div[data-slots]`（CSS `display:none` 兜底）；
- **新增通道 E**：隐藏静态图片广告——跨站绝对外链且内容仅为图片（本站横幅由 `/abc/fixed_ui_*.js` 动态注入跨站赌场图，`/abc/*.js`、`/000/` 已加入广告 URL 指纹，注入脚本被网络层拦截）；
- 修复折叠从未生效的历史 bug：折叠容器此前只设 `data-vir-nav` 属性而未加 `vir-nav` class，CSS 选择器永不匹配；
- LayoutEngine 兼容 PC 模板：栅格化取「img 卡片最多的容器」（PC 实测 48 张卡），标题跳过含图 `p.img`；分类折叠改用与识别一致的聚桶逻辑（兼容链接被逐个包裹的侧边栏）；
- 端到端验证扩展为 WAP + PC 双模板（jsdom × 真实 HTML × 真实脚本全链路）。
